...
def analyze_traditional_ranking_factors(df):
    ...
    if len(available_features) >= 3:
        ...
        correlations = feature_data.corr()['Included'].drop('Included').sort_values(key=abs, ascending=False)

        print(f"\n🔗 SEO FEATURE CORRELATIONS WITH AI OVERVIEW INCLUSION:")
        for feature, corr in correlations.items():
            significance = "***" if abs(corr) > 0.3 else "**" if abs(corr) > 0.1 else "*" if abs(corr) > 0.05 else ""
            print(f"   • {feature}: {corr:+.3f} {significance}")

        return correlations, available_features
    else:
...
def main():
    ...
    # Analyze SEO factors
    correlations, available_features = analyze_traditional_ranking_factors(df)
